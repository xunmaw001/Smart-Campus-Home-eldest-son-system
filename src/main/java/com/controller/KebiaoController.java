    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(kebiao !=null){
            ...
            BeanUtils.copyProperties( kebiao , view );//把实体数据重构到view中
                ...
                //级联表
                JiaoshiEntity jiaoshi = jiaoshiService.selectById(kebiao.getJiaoshiId());
                if(jiaoshi != null){
    ...
    */
    ...
    public R save(@RequestBody KebiaoEntity kebiao, HttpServletRequest request){
        ...
        if(false)
            ...
            kebiao.setJiaoshiId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));

        Wrapper<KebiaoEntity> queryWrapper = new EntityWrapper<KebiaoEntity>()
            .eq("banji_types", kebiao.getBanjiTypes())
            .eq("xingqi_types", kebiao.getXingqiTypes())
            .eq("jiaoshi_id", kebiao.getJiaoshiId())
            ;
...
//                            kebiaoEntity.setBanjiTypes(Integer.valueOf(data.get(0)));   //班级 要改的
//                            kebiaoEntity.setXingqiTypes(Integer.valueOf(data.get(0)));   //星期 要改的
//                            kebiaoEntity.setJiaoshiId(Integer.valueOf(data.get(0)));   //教师 要改的
